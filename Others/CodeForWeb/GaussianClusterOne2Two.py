...
def ReadData(path_In_Source, path_In_Text, path_In_Product, path_In_AllPicture, path_In_Class):
    pathSource = path_In_Source
    pathText = path_In_Text
    pathProduct = path_In_Product
    pathPicture = path_In_AllPicture
    pathClass = path_In_Class

    classGroup = []
    for line in open(pathClass, 'r'):
        classGroup.append(line[:-1])
    ...
    for each in picName:
        ...
        for item in each:
            ...
            for line in open(path1, 'r'):
                if k != 0:
                    if line[len(line) - 1] == '\n':
                        sGroup.append(line[:-1].split(' '))
                    else:
                        sGroup.append(line.split(' '))
                k += 1
            tData = []
            if os.path.getsize(path1) == 0:
                continue
            else:
                if (int(sGroup[0][1]) - int(sGroup[0][0])) * (int(sGroup[0][3]) - int(sGroup[0][2])) > (
                            int(sGroup[1][1]) - int(sGroup[1][0])) * (int(sGroup[1][3]) - int(sGroup[1][2])):
                    tData.append(
                        [int(sGroup[0][0]) / shape[1], int(sGroup[0][1]) / shape[1], int(sGroup[0][2]) / shape[0],
                         int(sGroup[0][3]) / shape[0]])
                    tData.append(
                        [int(sGroup[1][0]) / shape[1], int(sGroup[1][1]) / shape[1], int(sGroup[1][2]) / shape[0],
                         int(sGroup[1][3]) / shape[0]])
                else:
                    tData.append(
                        [int(sGroup[1][0]) / shape[1], int(sGroup[1][1]) / shape[1], int(sGroup[1][2]) / shape[0],
                         int(sGroup[1][3]) / shape[0]])
...
def ClusterTwo(path_In_Source, path_In_Text, path_In_Product, path_In_AllPicture, path_In_Class, path_Out_Product,
               path_Out_Text):
    TotalData = ReadData(path_In_Source, path_In_Text, path_In_Product, path_In_AllPicture, path_In_Class)
    data = TotalData[0]
    output = []
    outputP = []
    outputT = []
    ...
    for ccData in data:
        ...
        outputP.append([ProductPosition[kP][0], ProductPosition[kP][1], pl, pw])
        kP += 1
    f2.close()
    ...
    for ttData in NewData:
        TRPData = []
        for each in ttData:
            TRPData.append([(each[1][0][0] + each[1][0][1]) / 2 - ProductPosition[k][0],
                            (each[1][0][2] + each[1][0][3]) / 2 - ProductPosition[k][1]])
        k += 1
    ...
    for ttData in NewData:
        tpData = []
        for each in ttData:
            tpData.append([each[1][0][1] - each[1][0][0], each[1][0][3] - each[1][0][2]])
    ...
    for ttData in NewData:
        TRPData = []
        for each in ttData:
            TRPData.append([(each[1][1][0] + each[1][1][1]) / 2 - ProductPosition[k][0],
                            (each[1][1][2] + each[1][1][3]) / 2 - ProductPosition[k][1]])
        pp = kde.KernelDensity(kernel='gaussian', bandwidth=0.5).fit(TRPData)
        inputData = []
        for a1 in np.linspace(-1, 1, 100):
            for a2 in np.linspace(-1, 1, 100):
    ...
    for ttData in NewData:
        tpData = []
        for each in ttData:
            tpData.append([each[1][1][1] - each[1][1][0], each[1][1][3] - each[1][1][2]])
        pp = kde.KernelDensity(kernel='gaussian', bandwidth=0.5).fit(tpData)
        inputData = []
        for a1 in np.linspace(0, 1, 100):
            for a2 in np.linspace(0, 1, 100):
        ...
        tw = np.linspace(0, 1, 100)[index % 100]
        tl = np.linspace(0, 1, 100)[int((index - tw) / 100)]
        f3.write('\n' + str(TextPosition[kT][0][0]) + ' ' + str(TextPosition[kT][0][1]) + ' ' + str(
            TextRange[kT][0]) + ' ' + str(TextRange[kT][1]) + ' ' + str(TextPosition[kT][1][0]) + ' ' + str(
            TextPosition[kT][1][1]) + ' ' + str(tl) + ' ' + str(tw))
        outputT.append(
            [TextPosition[kT][0][0], TextPosition[kT][0][1], TextRange[kT][0], TextRange[kT][1], TextPosition[kT][1][0],
             TextPosition[kT][1][1], tl, tw])
        kT += 1
    f3.close()
...
path_In_Source = "G:/浙大实习/text_product聚类/SelectedPictureTwo.txt"
path_In_Text = "G:/浙大实习/text聚类/"
path_In_Product = "G:/浙大实习/text_product聚类/productCorner.txt"
path_In_AllPicture = "G:/浙大实习/allImages-8337/"
path_In_Class = "G:/浙大实习/text_product聚类/resultOne2TwoAngleClass.txt"
path_Out_Product = "G:/浙大实习/text_product聚类/One2TwoProduct.txt"
path_Out_Text = "G:/浙大实习/text_product聚类/One2TwoText.txt"

output = ClusterTwo(path_In_Source, path_In_Text, path_In_Product, path_In_AllPicture, path_In_Class, path_Out_Product,
                    path_Out_Text)
x = 1
