...
with open("G:/浙大实习/数据label/JsonDataForBanner.csv") as fr:
    ...
    for row in rows:
        ...
        if item['productsNum'] == 1:
            ...
            # if len(eh1) == 2:
            #     count += 1
            if len(eh1) == 1 and ((eh1[0][1] - eh1[0][0]) > shape[1] * th3 or (eh1[0][3] - eh1[0][2]) > shape[0] * th3):
    ...
    path = "G:/浙大实习/text_product聚类/resultOne2OneAngle.txt"
    fobj = open(path, 'w')
    fobj.write(str(len(ss)) + ' ' + str(nLabel))
    for each in ss:
        fobj.write('\n' + each[1] + ' x' + each[2])
    fobj.close()

    pathCenter = "G:/浙大实习/text_product聚类/resultOne2OneAngleCenter.txt"
