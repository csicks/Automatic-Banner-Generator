...
def ReadDataTwo(numC):
    path_S = "G:/浙大实习/text_product聚类/One2TwoText.txt"
...
path_des = "G:/浙大实习/text内部聚类/LayoutOne2Ratio/"
if os.path.exists(path_des):
    shutil.rmtree(path_des)
os.mkdir(path_des)
path_P = "G:/浙大实习/text_product聚类/One2TwoProduct.txt"
path_T = "G:/浙大实习/text_product聚类/One2TwoText.txt"
path_font = "G:/Font/"
for classNum in range(0, 8):
    ...
    TotalData1 = ReadDataTwo(classNum)[0]
    TotalData2 = ReadDataTwo(classNum)[1]
    ...
    for each in TargetTextGroup:
        ...
        for i in range(0, len(each[1])):
            for j in range(0, len(each[1][i])):
                if i == 0:
                    mapTBox1.append([(each[1][i][j][1] - each[1][i][j][0]) * (yT12 - yT11) / each[1][i][j][4], [i, j]])
                if i == 1:
                    mapTBox1.append([(each[1][i][j][1] - each[1][i][j][0]) * (yT22 - yT21) / each[1][i][j][4], [i, j]])
    ...
    for layout in SuitableTextGroup:
            ...
            if lTemp1[i][0][1] > lTemp1[i + 1][0][0] or lTemp1[i + 1][0][0] - lTemp1[i][0][1] < (
                ...
                if (lTemp1[i][0][1] - lTemp1[i][0][0]) / (lTemp1[i + 1][0][1] - lTemp1[i + 1][0][0]) > th_h_r:
                    ...
                    if (lTemp1[i + 1][0][1] - lTemp1[i + 1][0][0]) <= (lTemp1[i][0][1] - lTemp1[i][0][0]):
                        dd = (lTemp1[i + 1][0][1] - lTemp1[i + 1][0][0]) * th_d + lTemp1[i][0][1] - lTemp1[i + 1][0][0]
                    else:
            ...
            if lTemp2[i][0][1] > lTemp2[i + 1][0][0] or lTemp2[i + 1][0][0] - lTemp2[i][0][1] < (
                ...
                if (lTemp2[i][0][1] - lTemp2[i][0][0]) / (lTemp2[i + 1][0][1] - lTemp2[i + 1][0][0]) > th_h_r:
                    ...
                    if (lTemp2[i + 1][1] - lTemp2[i + 1][0][0]) <= (lTemp2[i][0][1] - lTemp2[i][0][0]):
                        dd = (lTemp2[i + 1][0][1] - lTemp2[i + 1][0][0]) * th_d + lTemp2[i][0][1] - lTemp2[i + 1][0][0]
                    else:
        ...
        for i in range(0, len(lTemp1)):
            ...
            if n12 >= n11 and n12 >= n13:
                xx = 1 / 2 * (xT12 - xT11) + xT11
            elif n11 >= n12 and n11 >= n13:
                xx = len(inputString[layout[1][0][i][1]]) / 2 * (yT12 - yT11) / layout[1][0][i][0][4] + xT11
            elif n13 >= n11 and n13 >= n12:
                xx = (xT12 - xT11) - len(inputString[layout[1][0][i][1]]) / 2 * (yT12 - yT11) / layout[1][0][i][0][
                    4] + xT11
            if yy < hP / 2:
        ...
        for i in range(0, len(lTemp2)):
            ...
            if n22 >= n21 and n22 >= n23:
                xx = 1 / 2 * (xT22 - xT21) + xT21
            elif n21 >= n22 and n21 >= n23:
                xx = len(inputString[layout[1][1][i][1]]) / 2 * (yT22 - yT21) / layout[1][1][i][0][4] + xT21
            elif n23 >= n11 and n23 >= n22:
                xx = (xT22 - xT21) - len(inputString[layout[1][1][i][1]]) / 2 * (yT22 - yT21) / layout[1][1][i][0][
                    4] + xT21
            if yy < hP / 2:
        ...
        while len(upGroup) != 0 or len(downGroup) != 0:
            for i in range(0, len(lTemp1)):
                ...
                height = int((layout[1][0][i][0][1] - layout[1][0][i][0][0]) * (yT12 - yT11) / layout[1][0][i][0][4])
                        ...
                        for jj in range(0, len(inputString[layout[1][0][i][1]])):
                            ...
                            if Overlap((wordGroup[jj][0], wordGroup[jj][1], wordGroup[jj][2], wordGroup[jj][3]),
                                ...
                                if wordGroup[jj][2] + wordGroup[jj][3] < xP1 + xP2 and stateWord == 0:
                                    ...
                                    break
                        for jj in range(0, len(inputString[layout[1][0][i][1]])):
                            draw.text((wordGroup[jj][2], wordGroup[jj][0]), inputString[layout[1][0][i][1]][jj],
                                      'black', font)
                    else:
                        tempString = inputString[layout[1][0][i][1]]
                        lNum = int(width / height)
                        while (len(tempString) - len(tempString) % (lNum - 1)) / (lNum - 1) + 1 <= layout[1][0][i][0][
                            4]:
                ...
                if state_Pic == 1 and layout[1][0][i] in downGroup:
                    ...
                    if layout[1][0][i][0][4] == 1:
                        ...
                        if n12 >= n11 and n12 >= n13:
                            ...
                            startP = (xT12 - xT11) - widthW
                        draw.text((xT11 + int(startP), yT11 + int(layout[1][0][i][0][0] * (yT12 - yT11))),
                                  inputString[layout[1][0][i][1]], 'black', font)
                    else:
                        tempString = inputString[layout[1][0][i][1]]
                        lNum = int(width / height)
                        while (len(tempString) - len(tempString) % (lNum - 1)) / (lNum - 1) + 1 <= layout[1][0][i][0][
            ...
            for i in range(0, len(lTemp2)):
                ...
                height = int((layout[1][1][i][0][1] - layout[1][1][i][0][0]) * (yT22 - yT21) / layout[1][1][i][0][4])
                rr = random.randint(1, 3)
                if layout[1][1][i][1] > 3:
                    font = ImageFont.truetype(path_font + str(3) + '/' + str(rr) + ".ttf", height, encoding="unic")
                else:
                        ...
                        for jj in range(0, len(inputString[layout[1][1][i][1]])):
                            ...
                            if Overlap((wordGroup[jj][0], wordGroup[jj][1], wordGroup[jj][2], wordGroup[jj][3]),
                                ...
                                if wordGroup[jj][2] + wordGroup[jj][3] < xP1 + xP2 and stateWord == 0:
                                    ...
                                    break
                        for jj in range(0, len(inputString[layout[1][1][i][1]])):
                            draw.text((wordGroup[jj][2], wordGroup[jj][0]), inputString[layout[1][1][i][1]][jj],
                                      'black', font)
                    else:
                        tempString = inputString[layout[1][1][i][1]]
                        lNum = int(width / height)
                        while (len(tempString) - len(tempString) % (lNum - 1)) / (lNum - 1) + 1 <= layout[1][1][i][0][
                            4]:
                            if lNum - len(tempString) % lNum > lNum - 1 - len(tempString) % (lNum - 1):
                                lNum = lNum - 1
                            else:
                ...
                if state_Pic == 1 and layout[1][1][i] in downGroup:
                    ...
                    if layout[1][1][i][0][4] == 1:
                        ...
                        if n22 >= n21 and n22 >= n23:
                            ...
                            startP = (xT22 - xT21) - widtW
                        draw.text((xT21 + int(startP), yT21 + int(layout[1][1][i][0][0] * (yT22 - yT21))),
                                  inputString[layout[1][1][i][1]], 'black', font)
                    else:
                        tempString = inputString[layout[1][1][i][1]]
                        lNum = int(width / height)
                        while (len(tempString) - len(tempString) % (lNum - 1)) / (lNum - 1) + 1 <= layout[1][1][i][0][
